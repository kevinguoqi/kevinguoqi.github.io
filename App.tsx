...
import { DataRow, SheetAnalysis } from './types';
...
const App: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const metaAnalysis = await analyzeDataWithGemini(parsedData.slice(0, 50));
  ...
  const comboPoints = useMemo(() => {
    const scores: Record<string, { points: number, parts: string[] }> = {};
    filteredData.forEach(row => {
      ...
      if (lock || main || assist) {
        partsList = [lock, main, assist, ratchet, bit].filter(p => p !== '');
      } else if (blade) {
        partsList = [blade, ratchet, bit].filter(p => p !== '');
      }

      if (partsList.length < 3) return;
      const comboKey = partsList.join(' ');
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 selection:bg-blue-500/30">
      <nav className="border-b border-slate-800 bg-slate-900/50 backdrop-blur-xl px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tighter text-white uppercase italic">BX-WBO Meta Hub</h1>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">WBO Intelligence Center</p>
          </div>
        </div>
              ...
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 space-y-6">
                    <div className="bg-gradient-to-br from-slate-900 to-slate-950 p-10 rounded-[2.5rem] border border-slate-800 relative overflow-hidden group shadow-2xl">
                      <h2 className="text-blue-500 font-black text-[10px] uppercase tracking-[0.3em] mb-6 flex items-center gap-3 italic">
                        <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                        Tactical Meta Analysis
                      </h2>
                      <p className="text-2xl font-semibold leading-relaxed text-slate-100 italic">{analysis?.summary || "Analyzing circuit shifts..."}</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {analysis?.insights.map((insight, idx) => (
                        <div key={idx} className="bg-slate-900/40 backdrop-blur p-8 rounded-3xl border border-slate-800/50 flex gap-6 hover:border-blue-500/30 transition-all">
                          <div className="w-10 h-10 rounded-xl bg-blue-600/10 border border-blue-600/20 flex items-center justify-center shrink-0 shadow-inner">
                             <span className="text-blue-500 font-black text-sm italic">{idx + 1}</span>
                          </div>
                  ...
                  <div className="space-y-6">
                    <div className="bg-gradient-to-tr from-blue-700 to-blue-600 p-10 rounded-[2.5rem] text-white shadow-2xl shadow-blue-900/40 relative overflow-hidden h-full">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.3em] mb-10 opacity-80 italic">Top Dominant Combos (CX Enabled)</h3>
                        <div className="space-y-6">
                          {comboPoints.slice(0, 10).map((combo, i) => (
                            <div key={combo.name} className="group border-b border-white/10 pb-6 last:border-0">
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center font-black italic text-xs">#{i+1}</div>
                                  <span className="font-black text-xl tabular-nums">{combo.points} <span className="text-[9px] opacity-60">PTS</span></span>
                                </div>
                              </div>
                              <div className="flex flex-wrap gap-x-2 gap-y-1 mt-3">
                                {combo.parts.map((part, pIdx) => (
                                  <a 
                                    key={`${combo.name}-${pIdx}`}
                                    href={getWikiUrl(part)} 
                                    target="_blank" 
                                    rel="noopener noreferrer" 
                                    className="text-[10px] font-black uppercase tracking-tight text-white/90 hover:text-cyan-300 underline decoration-white/20 underline-offset-4 transition-colors p-1"
                                  >
                                    {part}
                                  </a>
                                ))}
      ...
      <footer className="border-t border-slate-800 py-20 px-6 mt-20 text-center bg-slate-950 relative overflow-hidden">
        ...
        <p className="text-sm text-slate-600 font-medium leading-relaxed italic px-10 max-w-2xl mx-auto">
          Market Share grouping implemented for clarity: slices &lt; 8% merged into "Others". Data covers BX (3-part) and CX (5-part) systems.
        </p>
      </footer>
    </div>
  );
